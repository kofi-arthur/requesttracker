...
$fecthInfo = mysqli_query($conn, "SELECT * FROM `users` WHERE `email` = '$email' LIMIT 1;" );
...
if($fecthInfo && mysqli_num_rows($fecthInfo)) {
    ...
    if (password_verify($oldPassword, $result['password']) == 1) {
        ...
        $updatePassword = mysqli_query($conn, "UPDATE `users` SET `password` = '$encryptedPassword' WHERE `email` = '$email';");

        if($updatePassword && mysqli_affected_rows($conn) > 0) {
            header("Location: ../logout.php");
            $_SESSION['notify'] = $success;
        } else {
            header("Location: ../manageAccountC.php");
